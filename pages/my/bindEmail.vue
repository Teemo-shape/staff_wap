<template>
  <div>
    ...
    <div class="bind_content">
        <div>
          <img class="bind_img" src="/img/bindEmail.png">
          <div class="bind_txt" v-if="hasbind">
            <p class="bind_title">你已绑定邮箱：</p>
            <p class="bind_email">{{userinfo.bindMail}}</p>
          </div>
          <div class="bind_txt" v-else>
            <p class="bind_title">当前未绑定邮箱</p>
          </div>
          <div v-show="step1" class="btn btn_upate" @click="sendEmainCode">更换绑定邮箱</div>
          <div class="step2_veryfy" v-show="step2">
              <p class="bind_label">验证码发送至邮箱：<span>{{userinfo.bindMail}}</span></p>
          ...
          <div class="step3_confirm" v-show="step3">
              <p class="bind_label" id="bind_tip">验证通过！请输入您要绑定的邮箱：</p>
              <div class="form_item large_ipt">
                <input type="text" class="ipt_txt" v-model="newEmail" placeholder="请输入新邮箱">
              </div>
  ...
  export default {
    ...
    methods: {
        ...
        this.$http.get('/usercenter/tclcustomer/userInfo', {}, res => {
          if (res.data) {
            ...
            if(res.data.bindMail){
              ...
              _this.step2=false;
              _this.step3=true;
              document.getElementById('bind_tip').innerHTML='请输入您要绑定的邮箱';
            }
          }
        });
      },
      ...
      sendCode(){
        ...
        if(!regemail.test(that.newEmail)){
          that.$dialog.toast({
            mes: '请输入正确的邮箱',
            timeout: 1500,
            icon: 'error'});
